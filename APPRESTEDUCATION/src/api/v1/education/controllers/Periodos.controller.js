...
export const postPeriodosItem = async (req, res, next) => {
  try {
    ...
    const newPeriodosItem = await PeriodosService.postPeriodosItem(
      paPeriodosItem
    );
    if (!newPeriodosItem) {
...
export const putPeriodosItem = async (req, res, next) => {
  try {
    ...
    const updatedPeriodosItem = await PeriodosService.putPeriodosItem(
      id,
      puPeriodosItem
    );
...
export const pushStatus = async (req, res, next)=>{
  try {
    const { idPeriodo, idStatus}=req.params;
    const infoStatus= req.body;
    const pushStatus = await PeriodosService.pushStatus(
      idPeriodo,
      idStatus,
      infoStatus
    );
    if (!pushStatus.success) {
      throw boom.notFound(pushStatus.error);
    } else {
      res.status(200).json(pushStatus);
    }
  } catch (error) {
    next(error);
  }
...
export const getEstatus =async(req,res,next)=>{
  try {
    const { idPeriodo, idStatus}=req.params;
    const getEstatus = await PeriodosService.getEstatus(
      idPeriodo,
      idStatus
    );
    if (!getEstatus.succes) {
      throw boom.notFound(getEstatus.error);
    } else {
      res.status(200).json(getEstatus);
    }
  } catch (error) {
    next(error);
  }
...
export const deleteEstatus= async(req,res,next)=>{
  try {
      const{idPeriodo,idStatus}=req.params;
      const deleteEstatus = await PeriodosService.deleteEstatus(
        idPeriodo,
        idStatus
      );
      if (!deleteEstatus.succes) {
        throw boom.notFound(deleteEstatus.error);
      } else {
        res.status(200).json(deleteEstatus);
      }
  } catch (error) {
    next(error);
  }
